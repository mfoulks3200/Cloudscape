...
function start(port,debug) {
        log.log("Backend Server Starting on port "+port);
        var ports = port;
        exports.ports = ports;
  function onRequest(request, response) {
                ...
                log.log("Backend request for " + file + " received from "+request.connection.remoteAddress);
                  ...
                  if (exists) {
                        fs.readFile(file, "binary", function(err, file) {  
                                if(err) {  
                                        ...
                                        response.write(err + "\n");  
                                        response.end(); 
                                        log.warn("Backend request for " + fname + " from "+request.connection.remoteAddress+" returned with error code " + err);
                                        return;  
                                }
                                response.writeHead(200);  
                                response.write(file, "binary");  
                                response.end();  
                                log.log("Backend request for " + fname + " from "+request.connection.remoteAddress+" fufilled");
                        });  
                  } else {
                        response.writeHead(404, {"Content-Type": "text/html"});
                        response.write("Error 404: File not Found");
                        response.end();
                        log.warn("Backend request for " + fname + " from "+request.connection.remoteAddress+" could not be located");
                  }
                });
        }
        
        http.createServer(onRequest).listen(port);
        log.log("Backend started listening");
